...
$$ F_p = T_{\mathrm{max},p}u_p^2,\qquad F_v = T_{\mathrm{max},v}\left(u_f^2 + C_{pr}(V,u_p)u_r^2\right). $$
...
$$ \ddot{\theta} = \frac{1}{J}\left(\frac{1}{2}\rho S\bar{c}V^2 C_M + \frac{1}{4}\rho S\bar{c}^2 V C_{M_\omega}\omega + l_vT_{\mathrm{max},v}\left(u_f^2 - C_{pr}(V,u_p)u_r^2\right)\right). $$

The aerodynamic coefficients are

$$ C_D = C_{D_0} + C_{D_\alpha}\alpha^2 + C_{D_e}u_e + C_{D_t}(u_f+u_r). $$

$$ C_L = C_{L_e}u_e + (1-\sigma(\alpha))(C_{L_0}+C_{L_\alpha}\alpha) + \sigma(\alpha)2\,\mathrm{sgn}(\alpha)\sin^2(\alpha)\cos(\alpha). $$

$$ C_M = C_{M_e}u_e + (1-\sigma(\alpha))(C_{M_0}+C_{M_\alpha}\alpha) - \sigma(\alpha)2\,\mathrm{sgn}(\alpha)\sin^2(\alpha)\cos(\alpha). $$

The stall blending function is

$$ \sigma(\alpha) = \frac{1 + e^{-M(\alpha-\alpha_{\mathrm{stall}})} + e^{M(\alpha+\alpha_{\mathrm{stall}})}}{\left(1 + e^{-M(\alpha-\alpha_{\mathrm{stall}})}\right)\left(1 + e^{M(\alpha+\alpha_{\mathrm{stall}})}\right)}. $$
...
| $T_{\mathrm{max},v}$ | 100.0 | N | Maximum vertical propeller thrust scale |
| $T_{\mathrm{max},p}$ | 50.0 | N | Maximum pusher propeller thrust scale |
...
| $\alpha_{\mathrm{stall}}$ | 15.0 | deg | Stall blending angle |
