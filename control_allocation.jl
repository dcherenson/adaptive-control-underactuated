...
const n_u = 7 # augmented control input + 2 slack variables dimension
const n_s = 2
...
@kwdef struct ControlAllocationParams
    ...
    opti_weight::SMatrix{n_u-n_s,n_u-n_s, Float64} = Diagonal(@SVector[10.0, 10.0, 1.0, 0.1, 50.0])
    Γ_uλ::Float64 = 50.0
...
function ϕ(x,u,pitch,p::VTOLParams)
    ...
    return M\[
        ...
        [(@SMatrix zeros(1,5)) M_aero p.l_motor*p.max_thrust_vert*u[2]^2*u[3]^2]
        ]
...
function J(t,u::AbstractVector, c::ControlAllocationParams, r::RefTrajParams)
  ...
  quad_cost =  0.5*dot(u_diff, c.opti_weight*u_diff)                     # ½‖u‖²
...
function dL_du_jacobian(t, x, u, λ, W, xhat,s)
  return ForwardDiff.jacobian(vcat(x,u,W,xhat,@SVector[t])) do v
      x = v[SA[1:n_x...]]
      u = v[SA[n_x+1 : n_x+n_u...]]
      W = v[SA[n_x+n_u+1 : n_x+n_u+n_W...]]
      xhat = v[SA[n_x+n_u+n_W+1 : n_x+n_u+n_W+n_x...]]
      t = v[end]
    return dL_du(t, x, u, λ, W, xhat, s)
  end
end

function dL_dλ_jacobian(t, x, u, λ, W, xhat, s)
  return ForwardDiff.jacobian(vcat(x,u,W,xhat,@SVector[t])) do v
      x = v[SA[1:n_x...]]
      u = v[SA[n_x+1 : n_x+n_u...]]
      W = v[SA[n_x+n_u+1 : n_x+n_u+n_W...]]
      xhat = v[SA[n_x+n_u+n_W+1 : n_x+n_u+n_W+n_x...]]
      t = v[end]
    return constraint(t, x, u, W, xhat, s)
  end
end
